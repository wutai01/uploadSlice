...
app.all('/merge', (req, resp) => {
    ...
    let size = query.size
    let fileName = query.fileName
    mergeFiles(path.join('nodeServer/uploads', md5), uploadDir, fileName, size)
    resp.send({
        stat: 1
    })
})
...
app.all('/upload', (req, resp) => {
    folderIsExit('nodeServer/tmp').then(() => {
        ...
        form.parse(req, function(err, fields, file) {
            ...
            let folder = path.join('nodeServer/uploads', fileMd5Value)
...
async function mergeFiles(srcDir, targetDir, newFileName, size) {
    ...
    concat(fileArr, path.join(targetDir, newFileName), () => {
        ...
        deleteall("nodeServer/tmp")
    })
}

app.listen(5200, () => {
    console.log('服务启动完成，端口监听5200！')
})
